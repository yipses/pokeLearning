...
- Seeded 29 entries. **Correction to what was originally written here:** this said they came "from documented pronunciations," which overstated it — the plan had been to source them from Bulbapedia, that fetch turned out to be blocked by the environment's egress proxy, and they were written from recall instead without the claim being revised. Recall and sourced are different confidence levels and shouldn't have been conflated.
- They're also unverified **by ear** — this sandbox has no speech voices installed at all (`getVoices()` returns empty) — so some may be "fixing" names that were already fine.
- **Triaged which names actually need respelling at all.** A blanket pass over all 1,021 would be counter-productive: a respelling only does anything when it differs from what the synthesiser already produces, and for an ordinary English-readable spelling (Bulbasaur, Charmander, Snorlax) it can only introduce risk. Scored every name for orthographic hazard instead — Japanese vowel runs (`ou`, `ei`, `ui`), soft/hard `g` and `c`, `ch` ambiguity, `y` as a vowel, rare initial clusters, non-letters, length. **297 names (29%) are at genuine risk; 724 look safe and are deliberately left alone.**
- **Prioritised Gen 1**, since catching is generation-gated and that's where the collection actually is. 35 of Gen 1's names were at risk; verified and added 19 of them (psyduck → "sy duck", gengar → "gang gar", scyther → "sy ther", raichu → "rye chew", pidgey → "pidj ee", tangela → "tang guh luh", and so on). Map is now 48 entries, 30 of them checked.
- **Sourcing is the bottleneck, not effort.** Every site with a bulk pronunciation guide — Bulbapedia, Serebii, PokémonDB, Wikipedia, the Fandom wikis — is blocked by this environment's egress proxy. `raw.githubusercontent.com` is reachable but no published dataset pairs Pokémon names with pronunciations. Web *search* works and returns roughly 4–5 confirmed names per query, so full coverage of the 297 at-risk names is on the order of 60–75 searches, and the hit rate falls off sharply for obscure species that no guide documents.
- **Later found that web *search* reaches the wikis even though direct fetch is blocked**, and used it to check 11 of the 29 against published guides. Nine held up; two were wrong and are now fixed: Rayquaza was `"ray kwah zuh"`, correctly `"ray kway zuh"` (English *qua* → *kway*, as in equator), and Gyarados was `"gyah ruh dose"`, correctly `"gair uh dose"`. Verified entries now carry a `// checked` marker so provenance is visible in the code rather than only in this log. 18 remain unchecked.
...
**Then ground it out to completion — 251 entries covering all 297 at-risk names.** 157 are `// checked` against published guides; 94 are `// unverified`, derived from each name's etymology because no guide could be found. The remaining ~46 at-risk names needed nothing: their hyphens are handled in code (below), or the hazard flag was a false positive on an ordinary English spelling.

- **De-hyphenation moved into `sayAs()` rather than 21 map entries.** Roster names are slugs, so multi-word species arrive as `great-tusk`, `roaring-moon`, `iron-bundle`. Those are plain English once the hyphen is a space, and a synthesiser handed a hyphen may spell it out or run the words together. One rule covers every current hyphenated name and any added later; the override map still wins where a name needs more than de-hyphenation (`tapu-koko`, `mr-rime` → "mister rime", `mime-jr` → "mime junior", `sirfetchd` → "sir fetched").
- **The unverified 94 are mostly puns a synthesiser cannot see** — Polteageist is poltergeist + tea, Eiscue is ice + cue, Toedscool is toads + Tentacool, Dudunsparce is Dunsparce doubled, Farigiraf is a giraffe palindrome. The etymology gives high confidence in the target even without a guide, but they are guesses and are labelled as such.
- **Sourcing died out around Gen 8.** Search returned solid pronunciations for Gens 1–7 at roughly 4–5 names a query, then degraded to returning links to the (blocked) guides rather than their content. Gen 9 in particular has little published material. That boundary is visible in the data: nearly all `checked` entries are Gen 1–7, nearly all `unverified` are Gen 8–9.
- **The audit tool now shows provenance** — a green `checked` or amber `unverified` chip per row, plus an "Unverified guesses (94)" filter, so an ear-audit can start with the entries that most need one. It parses the trailing comments out of the app's own source, since `eval` sees values but not comments.

**First real ear-audit came back, and it reshaped the map.** 39 names reported from actual play: 13 with a respelling typed straight into the tool, 26 flagged as wrong (including Magikarp, where the fix given *was* the plain name). Applied the 13 as `checked` — a device the child actually uses beats a published guide — and **dropped all 26 rather than guessing again**, letting the synthesiser read the real spelling. That returns them to a known state instead of trading one blind guess for another; if any still sound wrong unaided they'll resurface on the next listen and can be fixed specifically.

The pattern in the corrections is worth recording: nearly every rejected entry was **over-syllabified**. "jer af ah rig", "toe geh pee", "poo chee yen ah", "ar kuh nine" — splitting a name the engine already handled into chunks it then read unnaturally. The accepted fixes run the other way, toward fewer and blunter syllables ("wah chog", "gee oh dood", "sim ee sage", "rejeerock"). Lesson for the remaining 87 unverified entries: the default should be to leave a name alone, and a respelling should exist only where the plain spelling is genuinely misread — not merely where it *looks* like it might be.
