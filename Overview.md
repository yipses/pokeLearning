...
- **Full Spelling** — the mystery Pokémon's artwork is shown and the player spells its name from shuffled letter tiles (tap) or the keyboard. Controls: 🔊 (a speaker on the picture, speaks the name), 💡 Hint (reveals the next correct letter; once the allowance is spent the button becomes **New word**, and pressing it swaps in a different word from the same pool rather than leaving the player stuck), Backspace, Clear. Wrong letters are rejected immediately with a shake; correct letters lock into a slot.
