...
| **Pokédex** | A ✕, the HUD, generation tabs, then one generation's grid — caught in colour, uncaught as grey silhouettes |
...
- **The screen opens straight onto the collection.** A ✕ and the HUD, then the tabs, then the grid — the first cell sits about 195px down at every supported width. It used to carry a full card headed *"My Pokédex"* with a `60 / 1021 caught` pill under it, which pushed the grid most of a screen down to state a total nobody is working toward: the generation is the unit being filled, and its own header already says where it stands.
- **Closing is a ✕, not "← Back"**, matching the round screen. Same control, same corner, same meaning — the only difference was which screen you were on.
- **The HUD comes with it.** Rounds today and the streak follow you here rather than living only on the home screen, so the two numbers that measure a day's play do not disappear the moment you go looking at what you caught. The collection counter is not among them: this screen *is* the collection, and the generation header states it exactly.
...
The HUD is on the home screen, the results screen and the Pokédex. **Not on a round**: that has its own progress bar and ✕, and a second row of counters there would be two things to read at once.
