...
- **A pity timer caps droughts.** A pure per-question roll can go cold for a long stretch, which to a young child reads as the feature being broken rather than unlucky, so an encounter is *guaranteed* once the drought reaches **twice the average wait** — 20 questions at 10%, 4 at 50%. Twice the mean, rather than anything closer to it, is what keeps the Settings number honest: the measured rate stays within a few points of what's set (10% measures 11%, 50% measures 53%). The counter **resets at the start of every round** — each round is its own experience, so a cold streak never follows the child into the next one.
